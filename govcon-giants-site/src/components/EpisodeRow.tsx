...
interface EpisodeRowProps {
  episode: Episode;
  index?: number;
  /** Index into the episodes array — the title links to /podcast/<episodeIndex>. */
  episodeIndex?: number;
  className?: string;
}
 ...
 */
export default function EpisodeRow({ episode, index = 0, episodeIndex, className }: EpisodeRowProps) {
    ...
    >
      <div className="flex flex-col gap-6 sm:flex-row sm:items-start sm:gap-8">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mb-3 flex items-center gap-2 font-sans text-sm font-semibold uppercase tracking-[0.2em]">
          ...
          {episodeIndex !== undefined ? (
            <Link to={`/podcast/${episodeIndex}`}>
              <h3 className="font-display text-[24px] font-bold leading-[1.15] tracking-normal text-slate-900 dark:text-white transition-colors hover:text-brand md:text-[28px]">
                {episode.title}
              </h3>
            </Link>
          ) : (
