          ...
          {faqs.map((f, i) => (
            ...
            >
              <AccordionItem value={`item-${i}`} className="border-line">
                <AccordionTrigger className="text-[17px] font-medium text-slate-900 dark:text-white hover:text-brand hover:no-underline [&>svg]:text-brand">
                  {f.q}
                </AccordionTrigger>
                <AccordionContent className="text-[15px] leading-[1.7] text-slate-600 dark:text-slate-300">
                  {f.a}
                </AccordionContent>
              </AccordionItem>
            </motion.div>
