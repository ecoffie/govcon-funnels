...
export interface Guide {
  ...
  modalCopy: string;
  icon: LucideIcon;
  /** CRM tag suffix (lead arrives as guide-<slug>). */
  slug: string;
  /** Instant-access destination on govcongiants.com — shown after signup so
   *  the promise is kept immediately (no email automation exists for these). */
  href: string;
}
