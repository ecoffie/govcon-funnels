...
describe('SPA apex redirects for Next-owned funnels', () => {
  ...
  it('does not swallow the SPA /resources index with the handouts redirect', () => {
    const resourcesIndex = redirects.find((r) => r.source === '/resources');
    expect(resourcesIndex).toBeUndefined();
  });
});
