...
// Sentence-leading words that begin a takeaway bullet in the feed copy.
//
// The show notes are written in two house styles: an imperative list ("Learn the
// difference…", "Discover a hidden trick…") and a "Key discussion points from this
// episode:" list whose items open with an interrogative ("Why the real CMMC audit
// costs…", "How 75 percent of small businesses…"). The original SPA regex only knew
// the first style, so takeaways parsed for 25 of 150 episodes and the rest silently
// rendered no list at all.
//
// Interrogatives are matched only in their "Why/How/What <clause>" bullet form. The
// 2-or-more guard at the end of parseEpisodeBody still applies, so a stray mid-sentence
// match cannot produce a one-item section.
const LEAD =
  /(?<=[.\s])(Learn how|Learn|Discover|Use|Confirm|Build|Understand|Find out|Find|Get|Hear|Know|Master|Avoid|Uncover|Reveal|Explore|Steal|See how|See why|Why the|Why a|Why this|Why small|Why most|Why you|How the|How a|How this|How one|How small|How to|What the|What a|What this|What every)\b/g;
...
export function parseEpisodeBody(body: string): ParsedEpisode {
  ...
  // Some feed copy runs bullets together with no punctuation between them, so a
  // "takeaway" can swallow several. Those read as a wall of text rather than a list, so
  // drop the run-ons and keep the clean items. Showing four good bullets is better than
  // six where two are paragraphs — and better than inventing boundaries that aren't in
  // the source.
  const wellFormed = takeaways.filter((t) => t.length <= 300);

  return {
    intro: splitParagraphs(intro),
    // only surface takeaways when the split clearly worked (2+ clean items)
    takeaways: wellFormed.length >= 2 ? wellFormed : [],
    chapters,
    links,
  };
}
