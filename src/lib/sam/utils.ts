  ...
  for (const apiKey of apiKeys) {
    ...
    if (result.error) {
      ...
      if (result.error.fallbackAvailable) {
        console.log(`[SAM API] Key ${apiKey.slice(0, 10)}... failed with retryable error, trying backup...`);
        continue;
      }

      // Non-retryable request errors should not be retried against backup keys.
