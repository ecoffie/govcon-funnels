...
export async function GET(request: NextRequest) {
  ...
  if (result.error) {
    return NextResponse.json(
      {
        error: result.error.message || 'SAM.gov API is temporarily unavailable. Please try again later.',
      },
      {
        // Preserve explicit throttling; otherwise surface as upstream outage.
