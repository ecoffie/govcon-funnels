...
import { saveLeadToSupabase, getLeadSignupPosition, MINDY_LAUNCH_ZOOM_CAP } from '@/lib/supabase-leads';
...
export async function POST(request: NextRequest) {
  try {
    ...
    //     first persisted signup position so the thank-you page can show Zoom
    //     (first N) vs YouTube. Duplicate submissions keep their original rank.
    let mindyLaunch: { position: number; getsZoom: boolean; zoomCap: number } | null = null;
    if (lead.source === 'mindy-launch') {
      const position = await getLeadSignupPosition('mindy-launch', lead.email);
      if (position !== null) {
        mindyLaunch = {
          position,
