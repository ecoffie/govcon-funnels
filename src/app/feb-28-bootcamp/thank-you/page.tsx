...
export default function Feb28ThankYou() {
  const [userName, setUserName] = useState('Friend');

  useEffect(() => {
    const fromLeadName = localStorage.getItem('leadName');
    if (fromLeadName) {
      setUserName(fromLeadName.split(' ')[0]);
      return;
    }
    const leads = JSON.parse(localStorage.getItem('govcon_leads') || '[]');
    const last = leads[leads.length - 1];
    if (last?.name) {
      setUserName(last.name.split(' ')[0]);
    }
  }, []);
      ...
      <section className="py-16 px-6">
        <div className="max-w-2xl mx-auto text-center">
          ...
          <p className="text-xl text-slate-500 mb-8">
            Check your email for your bid forms and February 28th bootcamp details. We&apos;ve sent you everything you need for the Specifics and Proposals bootcamp.
          </p>

          {/* CTA Button */}
          <Link
            href="/"
            className="inline-block px-10 py-5 bg-green-600 hover:bg-green-500 text-white rounded-xl font-bold text-xl transition-all shadow-lg shadow-green-900/50"
          >
            Back to Home
          </Link>
          ...
          <div className="mt-12 bg-slate-900/50 border border-slate-800 rounded-xl p-6">
            <div className="flex items-center justify-center gap-3 text-slate-400">
              ...
              <span>We&apos;ve sent you an email with your bid forms and bootcamp access. Check your inbox!</span>
            </div>
          </div>
        </div>
      </section>
