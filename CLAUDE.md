...
**HUBZone address checker (commit `dc76a66`, verified live)** — `/guides/hubzone-certification` now answers its own search intent ("hubzone map/checker/by zip code" = the bulk of its 17.7K impr): `HubzoneCheckWidget` under the hero + `/api/hubzone-check`, which proxies the SBA map's own endpoint (`maps.certify.sba.gov/hubzone/map/search` — MUST send `X-Requested-With: XMLHttpRequest` + `Accept: application/json…` + `query_date`/`locale` params or it 406s) and extracts the `JSON.parse('…')` blob embedded in its jQuery payload (no HTML scraping). Unfindable address → 404 error, never a false "Not Qualified". Live-verified: Camden NJ → Qualified Census Tract; DC → Not Qualified; garbage → address-not-found.

**GA4 traffic overlay still blocked (needs Eric):** `GA4_PROPERTY_ID` unset, and the `mindy-bq-reader` SA can neither list GA4 properties (Analytics Admin API not enabled on `market-assasin`) nor enable that API (read-only role). Fix = Eric grabs the numeric property id from GA4 Admin → Property Settings (and grants the SA viewer on the property if it isn't already), then `vercel env add GA4_PROPERTY_ID` + redeploy.
